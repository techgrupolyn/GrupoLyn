...
export default function TemplatesView() {
  ...
  return (
    <div className="ceo-page p-4 sm:p-6 xl:p-8">
      ...
      <form onSubmit={save} className="ceo-card mt-8 grid grid-cols-1 gap-4 rounded-md border border-[#2E2E2E] bg-[#141414] p-5 lg:grid-cols-4">
        <label className="text-xs text-[#737373]">
          Nombre
          <input value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} className="mt-2 h-10 w-full ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none" required />
        </label>
        <label className="text-xs text-[#737373]">
          Lenguaje
          <input value={form.language} onChange={(e) => setForm((s) => ({ ...s, language: e.target.value }))} className="mt-2 h-10 w-full ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none" />
        </label>
        <label className="text-xs text-[#737373] lg:col-span-1">
          Tipo
          <select value={form.tipo} onChange={(e) => setForm((s) => ({ ...s, tipo: e.target.value }))} className="mt-2 h-10 w-full ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none">
        ...
        <div className="flex items-center gap-3 lg:col-span-4">
          ...
          {form.id && (
            <button type="button" onClick={() => setForm({ id: null, name: '', language: 'es', components: '', tipo: 'template' })} className="ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-4 py-2 text-xs font-medium text-[#F2F2F2]">
              Cancelar
            </button>
          )}
              ...
              <tr key={item.id} className="hover:bg-[#0D0D0D]/60">
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-2">
                    <button type="button" onClick={() => edit(item)} className="ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-2 py-1 text-[10px] uppercase tracking-widest text-[#F2F2F2] hover:bg-[#141414]">Editar</button>
