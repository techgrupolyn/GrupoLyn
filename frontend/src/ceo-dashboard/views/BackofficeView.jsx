  ...
  return (
    <div className="ceo-page p-4 sm:p-6 xl:p-8">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <form onSubmit={crearRol} className="ceo-card rounded-md border border-[#2E2E2E] bg-[#141414] p-5">
        ...
        <div className="ceo-card rounded-md border border-[#2E2E2E] bg-[#141414] p-5">
          <p className="text-[11px] font-medium uppercase tracking-[0.3em] text-[#737373]">Roles existentes</p>
          <ul className="mt-3 max-h-64 space-y-2 overflow-y-auto">
            {roles.map((r) => (
              <li key={r.id} className="flex items-center justify-between ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-4 py-2 text-xs text-[#F2F2F2]">
                <span>{r.nombre}</span>
        ...
        <form onSubmit={handleSubmit} className="ceo-card rounded-md border border-[#2E2E2E] bg-[#141414] p-5">
          <p className="text-[11px] font-medium uppercase tracking-[0.3em] text-[#737373]">Alta/edición de empleado</p>
          <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
            <input value={form.numero} onChange={(e) => setForm((f) => ({ ...f, numero: e.target.value.replace(/\D/g, '') }))} placeholder="Número" className="h-9 ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none placeholder:text-[#737373]" />
            <input value={form.nombre} onChange={(e) => setForm((f) => ({ ...f, nombre: e.target.value }))} placeholder="Nombre" className="h-9 ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none placeholder:text-[#737373]" />
            <input value={form.apellido} onChange={(e) => setForm((f) => ({ ...f, apellido: e.target.value }))} placeholder="Apellido" className="h-9 ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none placeholder:text-[#737373]" />
            <input value={form.empresa} onChange={(e) => setForm((f) => ({ ...f, empresa: e.target.value }))} placeholder="Empresa" className="h-9 ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none placeholder:text-[#737373]" />
            <select value={form.rol_id} onChange={(e) => setForm((f) => ({ ...f, rol_id: e.target.value }))} className="h-9 ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-3 text-xs text-[#F2F2F2] outline-none">
        ...
        <div className="ceo-card rounded-md border border-[#2E2E2E] bg-[#141414] p-5">
          ...
          <ul className="mt-3 max-h-64 space-y-2 overflow-y-auto">
            {employees.map((e) => (
              <li key={e.id} className="flex items-center justify-between ceo-surface rounded-md border border-[#2E2E2E] bg-[#0D0D0D] px-4 py-2 text-xs text-[#F2F2F2]">
