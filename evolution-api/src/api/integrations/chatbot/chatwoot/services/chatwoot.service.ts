  ...
  public async sendAttachment(waInstance: any, number: string, media: any, caption?: string, options?: Options) {
    try {
      ...
      if (!mimeType) {
        ...
        mimeType = typeof response.headers['content-type'] === 'string' ? response.headers['content-type'] : '';
      }
    ...
    try {
      ...
      if (event === 'messages.upsert' || event === 'send.message') {
        ...
        if (isAdsMessage) {
          ...
          const extension = mimeTypes.extension(typeof imgBuffer.headers['content-type'] === 'string' ? imgBuffer.headers['content-type'] : '');
