...
- Para permitir acceso con las mismas credenciales de Supabase sin copiar contraseñas, configura `SUPABASE_AUTH_ENABLED=true` junto con `SUPABASE_SOURCE_URL` y `SUPABASE_SOURCE_SECRET_KEY` **sólo en el backend**. La cuenta debe existir y estar activa en el directorio sincronizado; empleados reciben su rol y clientes quedan limitados a Consultas IA.
